...
function SectionTitle({ eyebrow, title, icon }: { eyebrow: string; title: string; icon: string }) {
  return <div className="premium-section-heading"><div><p className="premium-eyebrow text-violet-300/70">{eyebrow}</p><h2>{title}</h2></div><span className="premium-icon" aria-hidden="true">{icon}</span></div>;
}
  ...
  return (
    <main className="settings-screen theme-surface min-h-screen text-white">
      <div className="mx-auto w-full max-w-md px-5 pb-12 pt-5">
        ...
        <section className="mt-8"><span className="flex h-14 w-14 items-center justify-center rounded-2xl border border-violet-300/15 bg-violet-500/15 text-violet-200"><Icons.settings size={25} /></span><h1 className="mt-4 text-4xl font-black tracking-tight">Nastavenia</h1><p className="mt-2 text-sm leading-relaxed text-white/48">Vzhľad, zvuk a prístupnosť pre pohodlnú party na jednom mieste.</p></section>

        <section className="premium-card mt-7 p-4"><SectionTitle eyebrow="Vzhľad" title="Svetlo a téma" icon="🎨" />
          <div className="mt-4 grid grid-cols-2 gap-2 rounded-2xl border border-white/10 bg-black/20 p-1.5" role="group" aria-label="Farebný režim"><button type="button" aria-pressed={!settings.darkMode} onClick={() => update("darkMode", false)} className={`rounded-xl py-3 text-xs font-black transition ${!settings.darkMode ? "bg-white text-slate-900 shadow-lg" : "text-white/45"}`}>☀️ Svetlý</button><button type="button" aria-pressed={settings.darkMode} onClick={() => update("darkMode", true)} className={`rounded-xl py-3 text-xs font-black transition ${settings.darkMode ? "bg-slate-800 text-white shadow-lg" : "text-white/45"}`}>🌙 Tmavý</button></div>
        ...
        <section className="premium-card mt-4 p-4"><SectionTitle eyebrow="Zvuk" title="Odozva a hudba" icon="🔊" />
        ...
        <section className="premium-card mt-4 p-4"><SectionTitle eyebrow="Pohyb a prístupnosť" title="Pokojnejší zážitok" icon="♿" />
          ...
          <button type="button" data-feedback="off" disabled={!settings.vibrationEnabled} onClick={() => playFeedback("countdown")} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-cyan-300/20 bg-cyan-400/10 py-3 text-[10px] font-black text-cyan-100 disabled:opacity-35">📳 Vyskúšať vibráciu</button>
