            ...
            {phase.type === "question" && (
              <div className="grid grid-cols-2 gap-3">
                {participantNames.map((name, participant) => {
                  ...
                  return <button key={`${name}-${participant}`} disabled={!played} onClick={() => { playSound("buzzer"); stopAudio("ready"); setPhase({ type: "buzzed", participant }); }} className="party-shine overflow-hidden rounded-2xl py-5 text-base font-black text-white shadow-xl transition active:scale-95 disabled:opacity-30" style={{ background: color }}>🔔<span className="mt-1 block truncate px-2 text-sm">{name}</span></button>;
                })}
              </div>
            )}
            {phase.type === "buzzed" && (
