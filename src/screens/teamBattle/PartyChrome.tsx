...
export function PartyBackdrop({
  ...
  return (
    <div className={`party-backdrop fixed inset-0 overflow-hidden bg-[#070711] ${className}`}>
      ...
      <div className="party-grid absolute inset-0 opacity-20" />
...
export function PartyEyebrow({ children }: { children: ReactNode }) {
  return (
    <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-[#141a22]/85 px-3.5 py-2 text-[10px] font-extrabold uppercase tracking-[0.2em] text-white/58">
      <span className="h-1.5 w-1.5 rounded-full bg-[var(--game-accent,#a78bfa)]" />
      {children}
    </span>
  );
}
...
export function TeamBadge({
  ...
  return (
    <div
      className={`relative min-w-0 flex-1 overflow-hidden rounded-2xl border px-4 py-3 transition-all duration-300 ${active ? "scale-[1.02]" : ""}`}
      style={{
        borderColor: `${color}${active ? "bb" : "45"}`,
        background: `linear-gradient(145deg, ${color}${active ? "32" : "18"}, rgba(255,255,255,.035))`,
        boxShadow: active ? `0 14px 32px -22px ${color}, inset 0 1px 0 rgba(255,255,255,.09)` : "inset 0 1px 0 rgba(255,255,255,.05)",
      }}
    >
    ...
    <div className="flex gap-2 overflow-x-auto pb-1">
      {names.map((name, index) => {
        ...
        return (
          <div
            key={`${name}-${index}`}
            className={`min-w-[104px] flex-1 rounded-xl border px-3 py-2.5 text-left transition ${active ? "scale-[1.01]" : ""}`}
            style={{
