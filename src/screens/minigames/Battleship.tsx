  ...
  return <PartyBackdrop>
    <main className="flex h-full flex-col items-center justify-center overflow-y-auto px-6 py-10 text-center text-white">
      <div className="battle-handoff-icon relative flex h-36 w-36 items-center justify-center rounded-[2.5rem] border border-cyan-300/20 bg-gradient-to-br from-cyan-400/15 to-blue-700/20 shadow-2xl shadow-cyan-500/10">
        {toSecondPlayer ? <Icons.smartphone size={62} className="text-cyan-100" /> : <Icons.ship size={62} className="text-cyan-100" />}
...
export default function Battleship({ onBack }: { onBack: () => void }) {
    ...
    <main className="h-full overflow-y-auto px-5 pb-9 pt-5 text-white">
      <div className="mx-auto w-full max-w-md">
        ...
        <section className="mt-7 grid grid-cols-1 gap-3">
          <button onClick={() => startMode("local")} className="battle-local-mode party-shine relative flex items-center gap-4 overflow-hidden rounded-[1.7rem] border border-amber-300/20 bg-amber-400/[.08] p-5 text-left transition active:scale-[.97]"><span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-amber-200/15 bg-amber-300/[.1] text-amber-100"><Icons.users size={28} /></span><span><strong className="block text-lg font-black">Dvaja hráči</strong><small className="mt-1 block text-[11px] leading-relaxed text-white/48">Súkromné flotily na jednom mobile</small></span><Icons.chevronRight size={19} className="ml-auto text-amber-100/55" /></button>
        </section>
        <section className="battle-rules-card party-glass mt-4 rounded-[1.6rem] p-4"><div className="mb-2 flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-cyan-400/10 text-cyan-200"><Icons.shieldCheck size={15} /></span><p className="text-[9px] font-black uppercase tracking-[.2em] text-white/42">Pravidlá bitky</p></div><label className="flex cursor-pointer items-center justify-between gap-3 py-2"><span><strong className="block text-xs font-black">Lode sa nedotýkajú</strong><small className="text-[9px] text-white/35">Ani bokmi, ani rohmi</small></span><input type="checkbox" checked={noTouching} onChange={(event: ChangeEvent<HTMLInputElement>) => setNoTouching(event.target.checked)} className="battle-rule-check" /></label><label className="flex cursor-pointer items-center justify-between gap-3 border-t border-white/[.07] py-2"><span><strong className="block text-xs font-black">Zásah = ďalší výstrel</strong><small className="text-[9px] text-white/35">Úspešný strelec pokračuje</small></span><input type="checkbox" checked={hitKeepsTurn} onChange={(event: ChangeEvent<HTMLInputElement>) => setHitKeepsTurn(event.target.checked)} className="battle-rule-check" /></label></section>
      </div>
    </main>
  </PartyBackdrop>;
  ...
  if (phase === "deploying") return <PartyBackdrop><main className="flex h-full flex-col items-center justify-center px-6 text-center text-white"><div className="battle-radar relative flex h-44 w-44 items-center justify-center rounded-full border border-cyan-300/25 bg-cyan-400/[.06]"><span className="battle-radar-sweep" /><Icons.ship size={58} className="text-cyan-100" /></div><div className="mt-7"><PartyEyebrow>Čakanie na súpera</PartyEyebrow></div><h1 className="mt-5 text-3xl font-black">Flotily vyplávajú</h1><p className="mt-3 max-w-xs text-sm leading-relaxed text-white/45">Robot tajne rozmiestňuje svoje lode. Priprav delá!</p></main></PartyBackdrop>;
  ...
  if (phase === "placement") return <PartyBackdrop>
    <main className="battle-placement-screen h-[100dvh] overflow-hidden px-3 py-3 text-white">
      <div className="mx-auto flex h-full w-full max-w-md flex-col">
        ...
        <section className="battle-placement-tray mt-2 shrink-0 rounded-xl border border-white/[.08] bg-white/[.035] p-1.5">
          ...
          <div className="battle-ship-tray grid grid-cols-5 gap-1">{FLEET.map((definition) => {
            ...
            return <article key={definition.id} className={`battle-ship-block min-w-0 rounded-lg border p-0.5 transition ${placed ? "is-placed border-emerald-300/35 bg-emerald-400/[.12]" : "border-cyan-300/15 bg-cyan-400/[.045]"} ${selected ? "is-selected" : ""}`}>
              ...
              <button type="button" onClick={() => rotateShip(definition.id)} aria-label={`Otočiť ${definition.name} ${vertical ? "vodorovne" : "zvisle"}`} className="mt-1 flex w-full items-center justify-center rounded border border-white/[.07] bg-white/[.04] py-0.5 text-white/60"><Icons.refresh size={10} /></button>
            </article>;
          })}</div>
        </section>
        ...
        <div className="mt-2 grid shrink-0 grid-cols-2 gap-2"><button onClick={randomizeFleet} className="flex items-center justify-center gap-1.5 rounded-lg border border-white/10 bg-white/[.05] py-2 text-[8px] font-black uppercase tracking-wider text-white/60"><Icons.dice1 size={12} /> Náhodne</button><button onClick={clearFleet} className="rounded-lg border border-white/10 bg-white/[.05] py-2 text-[8px] font-black uppercase tracking-wider text-white/60">Vyčistiť</button></div>
