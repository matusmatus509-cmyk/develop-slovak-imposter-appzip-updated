...
interface Pair { id?: string; a: string; b: string; source: "bundled" | "custom" | "theme" }

export default function WouldYouRather({ onBack, customEntries = [], customControls, themedPrompts = [] }: {
  onBack: () => void; customEntries?: WorkshopEntry[]; customControls?: CustomContentControls; themedPrompts?: GeneratedPrompt[];
}) {
  const deck = useMemo<Pair[]>(() => [...WOULD_YOU_RATHER.map((item) => ({ ...item, source: "bundled" as const })), ...customEntries.filter((entry) => entry.answer).map((entry) => ({ id: entry.id, a: entry.text, b: entry.answer!, source: "custom" as const }))], [customEntries]);
  const themeQueue = useRef(themedPrompts.filter((item) => item.kind === "wouldRather" && item.answer).map((item) => ({ a: item.text, b: item.answer!, source: "theme" as const })));
  const draw = useCallback(() => themeQueue.current.shift() ?? takePersistentItem("would-you-rather", deck, (item) => item.source === "bundled" ? `${item.a}|${item.b}` : `${item.source}:${item.id ?? `${item.a}|${item.b}`}`), [deck]);
  ...
  return <Shell><TopBar title="Radšej by som..." onBack={onBack} /><div className="flex flex-1 flex-col items-center justify-center gap-5 text-center">
    ...
    <h1 className="text-xl font-black">Čo by si si vybral/a?</h1>{pair.source !== "bundled" && <span className="rounded-full bg-emerald-400/10 px-3 py-1 text-[9px] font-black text-emerald-300">{pair.source === "theme" ? "✨ AI Party téma" : "✨ Vlastná kartička"}</span>}
    <div className="flex w-full flex-col gap-3">{(["a", "b"] as const).map((option, index) => <div key={option}><button onClick={() => setPicked(option)} className={cn("w-full rounded-3xl border p-6 text-left transition active:scale-95", picked === option ? "border-fuchsia-400/70 bg-fuchsia-500/15" : "border-white/10 bg-white/5")}><span className="text-xs font-bold uppercase tracking-widest text-white/40">Možnosť {option.toUpperCase()}</span><p className="mt-2 text-lg font-bold">{pair[option]}</p></button>{index === 0 && <div className="mt-3 text-xs font-black uppercase tracking-widest text-white/30">alebo</div>}</div>)}</div>
