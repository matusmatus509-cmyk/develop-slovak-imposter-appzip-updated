  ...
  return (
    <main className="theme-surface party-hub-page min-h-screen text-white">
      <div className="mx-auto w-full max-w-md px-4 pb-20 pt-[max(1rem,env(safe-area-inset-top))]">
        ...
        <section className="premium-section"><div className="premium-section-heading"><div><p className="premium-eyebrow text-amber-300/70">Pripravené režimy</p><h2>Odporúčané balíky</h2></div><span className="premium-icon">🎁</span></div>
          <article className={`premium-card party-hover-card mt-3 p-4 ${level.level >= 20 ? "border-violet-300/25 bg-violet-400/[.09]" : ""}`}><div className="flex items-center gap-3"><span className="premium-icon">🎲</span><div className="min-w-0 flex-1"><h3 className="text-sm font-black">Bonusový mix · Level 20</h3><p className="mt-1 text-[9px] leading-relaxed text-white/40">Spustí jednu z odomknutých hier ako prekvapenie.</p></div><button type="button" disabled={level.level < 20} onClick={startBonusMix} className="rounded-xl bg-violet-300 px-3 py-2.5 text-[9px] font-black text-violet-950 disabled:bg-white/[.06] disabled:text-white/30">{level.level >= 20 ? "Spustiť" : "Zamknuté"}</button></div></article>
