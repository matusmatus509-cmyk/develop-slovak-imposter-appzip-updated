...
/** Design: Nočný herný salón — party hub používa hostovaný hero vizuál. */
import { partyHubHero } from "../media";
  ...
  return (
    <main className="theme-surface party-hub-page min-h-screen text-white">
      <div className="mx-auto w-full max-w-md px-4 pb-20 pt-[max(1rem,env(safe-area-inset-top))]">
        <header className="party-page-enter flex h-11 items-center justify-between border-b border-white/[.08]">
          <button type="button" onClick={onBack} className="text-[10px] font-bold text-white/55">Späť</button>
          <p className="text-[10px] font-black uppercase tracking-[.16em] text-white/72">Party Hub</p>
          <button type="button" onClick={() => onNavigate("settings")} className="text-[10px] font-bold text-white/55">Nastavenia</button>
        </header>

        <nav className="light-keep-dark party-page-enter sticky top-2 z-40 mt-3 grid grid-cols-3 overflow-hidden rounded-xl border border-white/10 bg-[#0d131a]/96 text-white" aria-label="Sekcie Party Hubu" style={{ animationDelay: "60ms" }}>
          {NAV_ITEMS.map(([id, label], index) => <button key={id} type="button" onClick={() => jumpTo(id)} className={`px-2 py-3 text-[9px] font-bold text-white/50 transition active:bg-white/[.05] ${index > 0 ? "border-l border-white/[.08]" : ""}`}>{label}</button>)}
        </nav>
        ...
        <section id="discover" className="light-keep-dark hub-hero party-hub-hero party-page-enter relative mt-4 scroll-mt-20 overflow-hidden rounded-[2rem] border border-white/12 p-5 shadow-2xl" style={{ animationDelay: "110ms" }}>
          ...
          <div className="relative z-10">
            <div><p className="text-[9px] font-bold text-white/48">Hra pre celú partiu</p><h1 className="mt-2 text-[2rem] font-black leading-none tracking-[-.05em]">Party mode</h1></div>
            <p className="mt-4 max-w-[17rem] text-[11px] font-semibold leading-relaxed text-white/55">Dva tímy, séria minihier a spoločné finále.</p>
            <button type="button" onClick={() => onNavigate("teambattle")} className="party-primary-action relative mt-5 w-full rounded-xl px-5 py-4 text-left text-white active:scale-[.98]">
              <strong className="block text-sm font-black">Pripraviť hru</strong>
            </button>
          </div>
        </section>

        <section className="party-page-enter mt-3 grid grid-cols-3 divide-x divide-white/[.08] rounded-xl border border-white/10 bg-[#111820] py-3" style={{ animationDelay: "170ms" }}>
          <button type="button" onClick={() => onNavigate("statistics")} className="text-center"><span className="block text-lg font-black">{level.level}</span><small className="mt-1 block text-[8px] font-bold text-white/38">Level</small></button>
          <div className="text-center"><span className="block text-lg font-black">{collections.length}</span><small className="mt-1 block text-[8px] font-bold text-white/38">Balíčky</small></div>
          <div className="text-center"><span className="block text-lg font-black">{safeStatistics.gamesPlayed}</span><small className="mt-1 block text-[8px] font-bold text-white/38">Odohrané</small></div>
        </section>
        ...
        <section className={`party-page-enter mt-3 rounded-[1.5rem] border p-4 ${dailyAvailable ? "border-amber-300/25 bg-amber-400/[.09]" : "border-emerald-300/20 bg-emerald-400/[.07]"}`} style={{ animationDelay: "220ms" }}>
          <div className="flex items-center gap-3"><div className="min-w-0 flex-1"><h2 className="text-sm font-black">Denný bonus · +{DAILY_REWARD_XP} XP</h2><p className="mt-1 text-[9px] text-white/40">{nextReward ? `Ďalšia odmena: ${nextReward.title}` : "Všetky odmeny odomknuté"}</p></div><button type="button" disabled={!dailyAvailable} onClick={onClaimDailyReward} className="rounded-lg bg-amber-300 px-3 py-2 text-[9px] font-black text-amber-950 disabled:bg-white/[.07] disabled:text-white/35">{dailyAvailable ? "Získať" : "Hotovo"}</button></div>
        </section>

        <section className="premium-section"><div className="premium-section-heading"><div><p className="premium-eyebrow text-cyan-300/70">Týždeň {weekly.number}</p><h2>Tip na tento týždeň</h2></div></div><article className="premium-card party-hover-card mt-3 overflow-hidden p-4"><div className="flex items-center gap-3"><div className="min-w-0 flex-1"><h3 className="font-black">{weekly.game.title}</h3><p className="mt-1 text-[10px] leading-relaxed text-white/48">{weekly.challenge}</p><p className="mt-2 border-l border-white/12 pl-3 text-[9px] leading-relaxed text-white/45">Otázka: {weekly.quiz}</p></div><button type="button" onClick={() => onNavigate(weekly.game.screen)} className="rounded-lg border border-white/15 px-3 py-2.5 text-[9px] font-black text-white/70">Otvoriť</button></div></article></section>
        ...
        <section className="premium-section"><div className="premium-section-heading"><div><p className="premium-eyebrow text-amber-300/70">Pripravené režimy</p><h2>Odporúčané balíky</h2></div></div>
          <article className={`premium-card party-hover-card mt-3 p-4 ${level.level >= 20 ? "border-violet-300/25" : ""}`}><div className="flex items-center gap-3"><div className="min-w-0 flex-1"><h3 className="text-sm font-black">Bonusový mix · Level 20</h3><p className="mt-1 text-[9px] leading-relaxed text-white/40">Náhodne vyberie jednu z odomknutých hier.</p></div><button type="button" disabled={level.level < 20} onClick={startBonusMix} className="rounded-lg border border-white/15 px-3 py-2.5 text-[9px] font-black text-white/70 disabled:text-white/25">{level.level >= 20 ? "Spustiť" : "Zamknuté"}</button></div></article>
          <div className="mt-3 grid grid-cols-2 gap-2.5">{PACKS.map((pack) => { const available = pack.status === "available"; return <article key={pack.id} className="premium-card party-hover-card p-3"><div className="flex items-center justify-end"><span className="text-[8px] font-bold text-white/35">{available ? "Dostupné" : "Čoskoro"}</span></div><h3 className="mt-2 text-xs font-black">{pack.title}</h3><p className="mt-1 min-h-9 text-[9px] leading-relaxed text-white/38">{pack.detail}</p>{available && "screen" in pack && <button type="button" onClick={() => onNavigate(pack.screen)} className="mt-2 text-[9px] font-black text-violet-300">Otvoriť</button>}</article>; })}</div></section>
        ...
        <button type="button" onClick={() => onNavigate("statistics")} className="premium-card party-hover-card mt-7 flex w-full items-center justify-between p-4 text-left"><span><span className="text-[9px] font-bold text-white/35">Achievementy</span><strong className="mt-1 block text-sm">{ACHIEVEMENTS.filter((achievement) => safeStatistics.progression.achievements[achievement.id]).length}/{ACHIEVEMENTS.length} odomknutých</strong></span><span className="text-[9px] font-bold text-white/45">Zobraziť</span></button>
      </div>
