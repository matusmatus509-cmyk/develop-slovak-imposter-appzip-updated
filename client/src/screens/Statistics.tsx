...
/** Design: Nočný herný salón — štatistiky ležia na tlmenom hostovanom povrchu herného stola. */
import { partyTableBackground } from "../media";
import {
  ACHIEVEMENTS,
  DAILY_CHALLENGES,
  DAILY_REWARD_XP,
...
export default function Statistics({ statistics, records, onBack, onClaimDailyReward }: { statistics: GameStatistics; records: PartyRecords; onBack: () => void; onClaimDailyReward: () => void }) {
  ...
  const metrics = [
    { label: "Odohrané hry", value: safeStatistics.gamesPlayed.toLocaleString("sk-SK") },
    { label: "Výhry tímov", value: totalTeamWins.toLocaleString("sk-SK") },
    { label: "Čas hrania", value: formatPlayTime(safeStatistics.totalPlaySeconds) },
    { label: "Správne odpovede", value: safeStatistics.correctAnswers.toLocaleString("sk-SK") },
  ];

  return (
    <main className="statistics-screen relative min-h-screen overflow-hidden bg-[#080b10] text-white">
      ...
      <div className="relative mx-auto min-h-screen w-full max-w-md overflow-y-auto px-5 pb-12 pt-5">
        <header className="flex h-11 items-center justify-between border-b border-white/[.08]">
          <button type="button" onClick={onBack} className="text-[10px] font-bold text-white/55">Späť</button>
          <span className="text-[10px] font-black uppercase tracking-[.16em] text-white/72">Profil</span>
          <span className="w-8" />
        </header>

        <section className="mt-9" style={{ animation: "slideUp .5s ease-out both" }}>
          <h1 className="text-[2.35rem] font-black leading-none tracking-[-.05em]">Vaše štatistiky</h1>
          <p className="mt-3 text-[11px] font-semibold text-white/44">Výsledky hier uložené v tomto zariadení.</p>
        </section>
        ...
        <section className={`relative mt-7 overflow-hidden rounded-[1.8rem] border p-5 shadow-[0_24px_70px_-38px_rgba(139,92,246,.9)] ${level.level >= 50 ? "border-amber-300/55 ring-2 ring-amber-300/15" : "border-violet-300/20"} ${level.level >= 10 ? "bg-[radial-gradient(circle_at_85%_0%,rgba(34,211,238,.18),transparent_42%),linear-gradient(145deg,#20123b,#101827)]" : "bg-[#151426]/95"}`} style={{ animation: "slideUp .5s ease-out 60ms both" }}>
          <div className="relative flex items-start justify-between gap-4">
            <div><p className="text-[9px] font-bold text-white/42">Aktuálny level</p><p className="mt-1 text-3xl font-black tracking-tight">Level {level.level}</p></div>
            <div className="text-right"><p className="text-lg font-black text-amber-300">{progression.coins.toLocaleString("sk-SK")}</p><p className="mt-1 text-[8px] font-bold text-white/35">mincí</p></div>
          </div>
        ...
        <section className="mt-4 rounded-[1.55rem] border border-amber-300/20 bg-amber-400/[.07] p-4" style={{ animation: "slideUp .5s ease-out 90ms both" }}>
          <div className="flex items-center gap-3"><div className="min-w-0 flex-1"><h2 className="text-sm font-black">Denný bonus · +{DAILY_REWARD_XP} XP</h2><p className="mt-1 text-[9px] font-semibold text-white/38">K dispozícii raz denne</p></div><button type="button" disabled={!dailyRewardAvailable} onClick={onClaimDailyReward} className="rounded-lg bg-amber-300 px-3 py-2 text-[9px] font-black text-amber-950 disabled:bg-white/[.07] disabled:text-white/35">{dailyRewardAvailable ? "Získať" : "Získané"}</button></div>
        </section>
        ...
        <section className="mt-7 grid grid-cols-2 gap-3" aria-label="Herné štatistiky">
          {metrics.map((metric, index) => <article key={metric.label} className="min-h-[108px] rounded-xl border border-white/[.1] bg-[#11171e]/95 p-4" style={{ animation: `slideUp .35s ease-out ${180 + index * 40}ms both` }}><p className="break-words text-[1.55rem] font-black leading-none tracking-[-.035em]">{metric.value}</p><p className="mt-3 text-[9px] font-bold text-white/38">{metric.label}</p></article>)}
        </section>
