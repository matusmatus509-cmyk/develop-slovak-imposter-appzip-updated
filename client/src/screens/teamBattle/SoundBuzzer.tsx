...
/** Design: Nočný herný salón — zvukový bzučiak používa hostovaný zvukový motív. */
      ...
      <main className="flex h-full flex-col overflow-hidden px-4 pb-4 pt-4 text-center">
        <div className="mx-auto flex min-h-0 w-full max-w-md flex-1 flex-col">
          <ParticipantScoreStrip
            ...
            activeIndex={
              phase.type === "question" ? undefined : phase.participant
            }
          />
          ...
          <section className="party-glass relative mt-3 flex min-h-0 flex-1 flex-col items-center justify-center overflow-hidden rounded-[2.2rem] px-6 py-5">
            ...
            <div className="party-fit-media relative h-40 w-full max-w-sm shrink overflow-hidden rounded-[1.8rem] border border-cyan-300/20 shadow-[0_0_70px_rgba(34,211,238,.18)]">
              ...
              <button
                onClick={
                  audioStatus === "playing" ? () => stopAudio("ready") : play
                }
              ...
              >
                <span
                  className={`flex h-24 w-24 items-center justify-center rounded-full border border-white/25 bg-black/40 text-5xl shadow-2xl backdrop-blur-md ${audioStatus === "loading" || audioStatus === "playing" ? "animate-pulse" : ""}`}
                >
                  {audioStatus === "loading" ? (
                    <Icons.hourglass size={38} />
                  ) : audioStatus === "playing" ? (
                    <Icons.stopCircle size={40} />
                  ) : played ? (
                    <Icons.music size={40} />
                  ) : (
                    <Icons.play size={40} />
            ...
            <p className="mt-2 max-w-xs text-sm leading-relaxed text-white/40">
              {audioStatus === "error"
                ? "Zvuk sa nepodarilo načítať. Skontrolujte internet a skúste ho prehrať znova."
                : phase.type === "question"
                  ? `Zvuk môžete prehrať znova. Keď ho ${playerWord} spozná, stlačí svoj bzučiak.`
                  : phase.type === "buzzed"
                    ? `${gameMode === "teams" ? "Tím" : "Hráč"} povie odpoveď nahlas a moderátor potom odkryje správny zvuk.`
              ...
              <div className="mt-4">
                ...
                {clue.sourcePage ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-3 block max-w-xs text-[8px] font-bold leading-relaxed text-white/25 underline decoration-white/15 underline-offset-2"
                  >
                    Zvuk: {clue.credit} · {clue.license} · Wikimedia Commons
                  </a>
                ) : (
                  <p className="mt-3 text-[8px] font-bold uppercase tracking-wider text-white/25">
          ...
          <div className="mt-4">
            {phase.type === "question" && (
              <div className="grid grid-cols-2 gap-3">
                {participantNames.map((name, participant) => {
                  const color =
                    PARTY_PLAYER_COLORS[
                      participant % PARTY_PLAYER_COLORS.length
                    ];
                  return (
                    <button
                      ...
                      onClick={() => {
                        stopAudio("ready");
                        playFeedback("buzzer");
                        setPhase({ type: "buzzed", participant });
                      }}
                      className="party-shine flex flex-col items-center justify-center overflow-hidden rounded-2xl py-5 text-base font-black text-white shadow-xl transition active:scale-95 disabled:opacity-30"
                      style={{ background: color }}
                    >
                      <Icons.bell size={20} />
                      <span className="mt-1 block truncate px-2 text-sm">
                        {name}
                      </span>
                    </button>
                  );
            ...
            {phase.type === "buzzed" && (
              <button
                ...
                className="party-shine w-full overflow-hidden rounded-2xl bg-gradient-to-r from-violet-600 to-cyan-500 py-5 text-base font-black text-white shadow-xl transition active:scale-95"
              >
                Ukázať správny zvuk
              </button>
            )}
            {phase.type === "revealed" && (
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="rounded-2xl bg-red-700 py-5 text-base font-black text-white transition active:scale-95"
                >
                  ✕ Nesprávne
                </button>
                <button
                  onClick={() => resolve(true)}
                  className="rounded-2xl bg-emerald-600 py-5 text-base font-black text-white transition active:scale-95"
                >
                  ✓ Správne
                </button>
              </div>
