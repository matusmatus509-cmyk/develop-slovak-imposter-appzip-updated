...
export default function PartyCollections({ collections, entries, autoImportNotice, onCollectionsChange, onEntriesChange }: {
  ...
  return (
    ...
    <section id="collections" className="premium-card p-4">
      <div className="premium-section-heading"><div><p className="premium-eyebrow text-emerald-300/75">Obsah iba v zariadení</p><h2>Vlastné kolekcie</h2></div><span className="premium-icon"><Icons.layout size={20} /></span></div>
      ...
      <div className="mt-4 rounded-2xl border border-violet-300/15 bg-violet-400/[.06] p-3">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">{SEASONAL_PARTY_PACKS.map((pack) => {
          ...
          return <article key={pack.id} className="rounded-xl border border-white/[.08] bg-black/15 p-3"><div className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-violet-400/10 text-violet-200"><Icons.package size={16} /></span><strong className="text-[10px] leading-tight">{pack.name}</strong></div><p className="mt-2 min-h-9 text-[8px] leading-relaxed text-white/38">{pack.description}</p><p className="mt-1 text-[8px] font-bold text-white/30">{pack.entries.length} kariet · viac hier</p><button type="button" disabled={installed} onClick={() => installSeasonalPack(pack)} className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-lg bg-violet-300 py-2 text-[9px] font-black text-violet-950 disabled:bg-emerald-300/20 disabled:text-emerald-200">{installed && <Icons.circleCheck size={13} />}{installed ? "Nainštalované" : "Nainštalovať"}</button></article>;
        })}</div>
      </div>
      ...
      <div id="collection-editor" className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-3">
        ...
        <fieldset className="mt-3"><legend className="premium-field-label">Kolekcie (jedna alebo viac)</legend><div className="flex flex-wrap gap-2">{collections.map((collection) => <button key={collection.id} type="button" aria-pressed={selectedCollectionIds.includes(collection.id)} onClick={() => toggleSelectedCollection(collection.id)} className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-[9px] font-black ${selectedCollectionIds.includes(collection.id) ? "border-emerald-300/40 bg-emerald-300 text-emerald-950" : "border-white/10 bg-white/[.04] text-white/45"}`}><Icons.layout size={13} /> {collection.name}</button>)}</div></fieldset>
