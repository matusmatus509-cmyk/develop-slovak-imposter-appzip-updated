...
export default function WouldYouRather({
  ...
  const deck = useMemo<Pair[]>(
    () => [
      ...getWouldYouRatherForLanguage(language).map(item => ({
        ...item,
  ...
  const draw = useCallback(
    () =>
      takePersistentItem("would-you-rather", deck, item =>
        item.source === "bundled"
          ? `${item.a}|${item.b}`
          : `${item.source}:${item.id ?? `${item.a}|${item.b}`}`
      ),
    [deck]
  );
  ...
  useEffect(() => {
    if (
      pair.source === "custom" &&
      !customEntries.some(entry => entry.id === pair.id)
    )
      next();
  ...
  const options: {
  ...
  }[] = [
    {
      key: "a",
      title: pair.a,
      color: "from-amber-400/25 via-orange-500/15 to-rose-500/20",
      muted: "border-amber-200/20",
      percent: shownA,
    },
    {
      key: "b",
      title: pair.b,
      color: "from-sky-400/20 via-indigo-500/15 to-violet-500/25",
      muted: "border-sky-200/20",
      percent: shownB,
    },
  ];

  return (
    <Shell className="would-rather-shell">
      <main
        className={cn(
          "relative mx-auto flex w-full max-w-xl flex-1 flex-col py-1 transition-all duration-150",
      ...
      >
              ...
              <div key={option.key}>
                <button
                  ...
                  className={cn(
                    ...
                    option.muted,
                    selected &&
                      "scale-[1.015] border-white/60 shadow-[0_22px_55px_-28px_rgba(255,255,255,.9)]",
                    otherSelected && "scale-[0.985] opacity-45 grayscale-[.25]",
                    !picked &&
                      "hover:-translate-y-0.5 hover:border-white/35 hover:shadow-[0_18px_40px_-28px_rgba(255,255,255,.75)] active:scale-[.985]"
                  )}
                >
                  ...
                  <div className="relative flex h-full flex-col items-center justify-center gap-5">
                    {selected && (
                      <span
                        className="absolute right-0 top-0 flex h-8 w-8 items-center justify-center rounded-full bg-white/20 text-lg font-black text-white"
                        aria-label="Vybraná možnosť"
                      >
                        ✓
                      </span>
                    )}
                    <p
                      className="max-w-[21ch] text-[clamp(1.45rem,6vw,2rem)] font-black leading-[1.13] tracking-tight text-white"
                      data-no-translate
                    >
                      {option.title}
                    </p>
                    {picked && (
                      <div className="flex items-center gap-3">
                        ...
                        <strong className="w-12 text-right text-xl font-black tabular-nums text-white">
                          {option.percent}%
                        </strong>
                      </div>
                    )}
                ...
                {index === 0 && (
                  <div className="relative z-10 -my-7 flex justify-center">
                    <span className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-white/45 bg-gradient-to-br from-rose-300/75 to-sky-300/75 text-xs font-black tracking-wide text-white shadow-[0_14px_34px_-14px_rgba(96,165,250,.9)] backdrop-blur-xl">
                      ALEBO
                    </span>
                  </div>
                )}
        ...
        {picked && (
          <div className="mt-1 animate-[fadeIn_.22s_ease-out]">
