...
import { forbiddenArt, fiveTenGameHero as fiveTenArt, letterGameHero as letterArt, musicQuizGameHero as musicQuizArt, soundArt } from "../../media";
...
export default function TeamQuickGame({
  ...
  const [names, setNames] = useState<string[]>(() => defaultNames("players", language));
  ...
  const playerRangeDescription = maxPlayers === 4 ? "2 až 4 hráči" : "2 až 8 hráčov";
  ...
  if (phase === "playing") {
    const shared = { participantNames: names, gameMode, rounds, timeSeconds, onDone: finish };
    ...
    if (game === "pismeno") return <LetterChallengeGame key={run} {...shared} />;
      ...
      <PartyBackdrop>
        <main className="flex h-full flex-col items-center overflow-y-auto px-5 py-8 text-center">
          ...
          <div className="mt-7 w-full max-w-sm space-y-3">
            ...
            <button onClick={() => { chooseMode("players"); setPhase("setup"); }} className="party-glass w-full rounded-2xl py-4 text-sm font-black text-white/70 transition active:scale-95">Nastavenia hry</button>
  ...
  return (
    <PartyBackdrop>
      <main className={`mobile-settings mobile-party-settings quick-setup-screen quick-setup-${game} h-full overflow-y-auto px-5 pb-8 pt-5`} style={{ "--quick-primary": theme.primary, "--quick-secondary": theme.secondary } as CSSProperties}>
        <div className="mx-auto w-full max-w-md">
          <header className="flex items-center justify-between">
            ...
            <PartyEyebrow>{gameMode === "teams" ? "Tímová hra" : "Nastavenia hry"}</PartyEyebrow>
          ...
          <div className="game-setup-hero relative mt-5 h-48 overflow-hidden rounded-[2rem] border border-white/15 shadow-2xl" style={{ "--setup-accent": accent } as CSSProperties}>
            ...
            <div className="absolute inset-x-5 bottom-5 flex items-end justify-between gap-3"><div><span className="quick-game-signature mb-3 inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[8px] font-black uppercase tracking-[.2em] backdrop-blur"><GameGlyph game={game} size={12} /> {theme.eyebrow}</span><h1 className="max-w-[16rem] text-[2rem] font-black leading-[.98] tracking-[-.04em] text-white">{GAME_TITLES[game]}</h1></div><span className="setup-player-badge inline-flex shrink-0 items-center gap-2 rounded-full border border-white/15 bg-black/35 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-white/80 backdrop-blur"><Icons.users size={15} /> {playerRange}</span></div>
          </div>

          <>
            <section className="party-glass mt-5 overflow-hidden rounded-[1.55rem]">
              <button type="button" onClick={() => setPlayersOpen((open) => !open)} aria-expanded={playersOpen} className="flex w-full items-center gap-3 p-4 text-left transition active:scale-[.99]">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl" style={{ background: `${accent}30`, color: accent }}><Icons.users size={22} /></span>
                <span className="min-w-0 flex-1"><strong className="block text-sm font-black text-white">{gameMode === "teams" ? "Tímy" : "Hráči"}</strong><small className="mt-0.5 block text-[11px] font-medium text-white/45">{gameMode === "teams" ? "2 tímy · spoločné body" : `${names.length} hráči · ${playerRangeDescription}`}</small></span>
                <span className="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider" style={{ color: accent }}>Upraviť <Icons.chevronRight size={16} className={playersOpen ? "rotate-90 transition-transform" : "transition-transform"} /></span>
              </button>
              {playersOpen && <div className="space-y-2 border-t border-white/[0.08] px-4 pb-4 pt-3">
                <p className="px-1 pb-1 text-[9px] font-black uppercase tracking-[0.2em] text-white/35">{gameMode === "teams" ? "Názvy tímov" : "Mená hráčov"}</p>
                {names.map((name, index) => {
                  const color = PARTY_PLAYER_COLORS[index % PARTY_PLAYER_COLORS.length];
                  return <label key={index} className="party-glass flex items-center gap-3 rounded-2xl p-3" style={{ borderColor: `${color}55` }}><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-sm font-black text-white" style={{ background: color }}>{gameMode === "teams" ? String.fromCharCode(65 + index) : index + 1}</span><input value={name} onChange={(event) => changeName(index, event.target.value)} maxLength={20} className="min-w-0 flex-1 border-0 bg-transparent text-base font-black text-white outline-none" />{gameMode === "players" && names.length > 2 && <button type="button" onClick={() => removePlayer(index)} aria-label={`Odstrániť ${name}`} className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/[0.06] text-lg text-white/40">×</button>}</label>;
                })}
                {gameMode === "players" && names.length < maxPlayers && <button onClick={addPlayer} className="party-glass w-full rounded-2xl py-3 text-sm font-black text-white/60 transition active:scale-95">+ Pridať hráča</button>}
              </div>}
            </section>

            <section className="party-glass mt-3 overflow-hidden rounded-[1.55rem]">
              <button type="button" onClick={() => chooseMode(gameMode === "teams" ? "players" : "teams")} aria-pressed={gameMode === "teams"} className="flex w-full items-center gap-3 p-4 text-left transition active:scale-[.99]">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl" style={{ background: gameMode === "teams" ? `${accent}30` : "rgba(255,255,255,.06)", color: gameMode === "teams" ? accent : "rgba(255,255,255,.5)" }}><Icons.users size={22} /></span>
                <span className="min-w-0 flex-1"><strong className="block text-sm font-black text-white">Tímový režim</strong><small className="mt-0.5 block text-[11px] font-medium text-white/45">Dva tímy · spoločné body</small></span>
                <span className={`relative h-7 w-12 rounded-full border transition ${gameMode === "teams" ? "border-white/20" : "border-white/10 bg-white/[.05]"}`} style={gameMode === "teams" ? { background: accent } : undefined}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow transition-transform ${gameMode === "teams" ? "translate-x-6" : "translate-x-1"}`} /></span>
              </button>
            </section>
