...
export const QUIZ_DUEL_QUESTIONS_BY_KIND: Record<QuizDuelKind, QuizDuelQuestion[]> = {
  classic: QUIZ_DUEL_CLASSIC_SK,
  estimate: QUIZ_DUEL_ESTIMATE_SK,
  closest: QUIZ_DUEL_CLOSEST_SK,
  "higher-lower": QUIZ_DUEL_HIGHER_LOWER_SK,
};
...
> {
  ...
  const coverage = {} as Record<AppLanguage, { translated: number; total: number }>;
  for (const language of languages) {
    const translated = QUIZ_DUEL_QUESTIONS.filter((question) =>
      localizableFields(question).every((field) => hasTranslation(field, language))
    ).length;
...
function deckKey(language: AppLanguage, difficulty: QuizDuelDifficulty, kind: QuizDuelKind) {
...
export function quizDuelPool(
  ...
  const all = [...QUIZ_DUEL_QUESTIONS_BY_KIND[kind], ...extra.filter((q) => q.kind === kind)];
